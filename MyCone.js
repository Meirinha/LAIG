	  ...
	  var levels = 1.0/this.stacks;
		...
		for (var i = 0; i<=this.stacks; i++)
			for (var j = 0; j<=this.slices; j++){
				this.vertices.push(Math.cos(j*ang)*((i/(Math.abs(this.bradius-this.tradius)/this.stacks))+1),Math.sin(j*ang)*((i/(Math.abs(this.bradius-this.tradius)/this.stacks))+1),i*levels);
